...
public class ProgletCreator {
  ...
  public static void mkdirProglet(String location) {
    ...
    if (!name.matches("[a-z][a-zA-Z][a-zA-Z][a-zA-Z]+"))
      throw new IllegalArgumentException("Le nom de la proglet «"+name+"» est bizarre il ne doit contenir que des lettres faire au moins quatre caractères et démarrer par une minuscule");
    if ((!new File(location).isDirectory()) && !new File(location).mkdirs()) {
      ...
      throw new RuntimeException("Impossible de créer le répertoire «"+location+"»de la proglet, "+tail);
    }
