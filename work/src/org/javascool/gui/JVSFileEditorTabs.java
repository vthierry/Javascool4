 ...
 */
class JVSFileEditorTabs extends JVSTabs implements FileEditorTabs {
  ...
  public void closeFile(String fileId) {
    if(this.getTabId(fileId) != -1) {      // We check if file is opened
      ...
      try {
        ...
        JVSMainPanel.getInstance().haveNotToSave(fileId);
      } catch(Exception e) {
  ...
  protected static void fileUpdateNotification() {
    JVSMainPanel.getInstance().mustSave(JVSMainPanel.getInstance().getEditorTabs().getCurrentFileId());
  }
  @Override
