...
if [[ "$1" == "--help" ]]
then
	echo Java\'s Cool Builder automated publication script
	echo Usage : /bin/sh publy.sh [proglet]
	echo Parameters : 
	echo -e "\t proglet \t The name of the proglet to publish"
	echo -e "\t         \t If not specified, publishes all proglets"
	exit
fi
...
then
	...
	do
		...
		echo -e "${status} \t <= \t ${proglet} ">>summary.log
		echo
	done
