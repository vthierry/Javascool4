...
public class Pml {
	...
	public Pml reset(Pml pml) {
		...
		if (pml != null) {
			setTag(pml.getTag());
			for (final String name : pml.attributes()) {
	...
	public Pml reset(String value, String format) {
		...
		if ("xml".equals(format))
			return reset(Xml2Xml.run(value, Pml.xml2pml), "pml");
	...
	public final Pml set(Properties pml) {
		for (final String name : pml.stringPropertyNames()) {
	...
	public Pml set(String name, Object value) {
		...
		if (value == null) {
			try {
				...
				final int i = Integer.parseInt(name), l = getCount() - 1;
				if (0 <= i && i <= l) {
					for (int j = i; j < l; j++) {
	...
	private static class PlainWriter {
		...
		private boolean write2d(Pml pml, int n, int tab) {
			...
			if (pml.getSize() == 0) {
				final boolean ln = n >= 0
						&& (n == 0 || pml.getParent() != null
								&& pml.getParent().getChild(n - 1) != null
	...
	protected static class TokenReader {
		...
		public String getToken(int next) {
			final String current = itoken + next < tokens.size() ? tokens
					.get(itoken + next).string : "}";
			return current;
		}
