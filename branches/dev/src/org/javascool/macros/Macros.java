...
public class Macros {
	...
	public static void assertion(boolean condition, String message,
		...
		if (!condition) {
			...
			// proglet
			{
				final StackTraceElement where[] = Thread.currentThread()
						.getStackTrace();
	...
	public static ImageIcon getIcon(String path) {
		final URL icon = Macros.getResourceURL(path); // Thread.currentThread().getContextClassLoader().getResource(path);
		if (icon == null) {
	...
	private static void openURL2(String location) {
		...
		try {
			if (os.indexOf("win") >= 0) {
				...
				final String[] browsers = { "epiphany", "firefox", "mozilla",
	...
	public static void sample(int delay, Runnable runnable) {
		final Sampler sampler = new Sampler().setDelay(delay).setRunnable(
				runnable);
		sampler.start();
	}
