...
public class LinkCheck {
	...
	private static void check(String location) {
		try {
			final String text = FileManager.load(location);
			LinkCheck.npages++;
	...
	public static String[] getAnchors(String text) {
		final HashSet<String> a = LinkCheck.getAnchorSet(text);
	...
	private static HashSet<String> getAnchorSet(String text) {
		...
		for (int i = 0, l = text.length(); i < l;) {
			final Matcher matcher = LinkCheck.anchorPattern.matcher(text)
					.region(i, l);
			if (matcher.find()) {
				final int i1 = matcher.end(), i2 = LinkCheck
						.nextQuote(text, i1);
	...
	public static String[] getLinks(String text) {
		...
		for (int i = 0, l = text.length(); i < l;) {
			final Matcher matcher = LinkCheck.linkPattern.matcher(text).region(
					i, l);
			if (matcher.find()) {
				final int i1 = matcher.end(), i2 = LinkCheck
						.nextQuote(text, i1);
				final String link = text.substring(i1, i2);
				if (!link.startsWith("'")) {
					h.add(link);
				}
				i = i2;
	...
	private static int nextQuote(String text, int i1) {
		final char c = text.charAt(i1 - 1);
		final int i2 = text.indexOf(c, i1);
