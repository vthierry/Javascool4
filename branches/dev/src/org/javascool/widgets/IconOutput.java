...
public class IconOutput extends JPanel {
	 ...
	 * 		// Ici ajouter les g.drawLine g.fillOval g.drawRect g.fillRect
	 * 		// souhaité.
	 * 	}
	 * }
	 * </pre>
	...
	public IconOutput reset(String location, boolean zoom) throws IOException {
		...
		for (int n = 0; n < 3; n++) {
			final BufferedImage img = ImageIO.read(Macros
	...
	private void setBounds() {
		final int di = width > 0 && getWidth() >= width && zoom ? getWidth()
				/ width : 1;
		final int dj = height > 0 && getHeight() >= height && zoom ? getHeight()
				/ height
				: 1;
		dij = di < dj ? di : dj;
