...
public class HtmlDisplay extends JPanel {
   ...
   * <p>
   * Cette routine est appelée pour gérer des URL specifiques d'une application
   * donnée.
   * </p>
   * 
  ...
  private void update(String location, boolean push) {
    ...
    if (location.startsWith(HtmlDisplay.stringPrefix)) { // Affichage de
      ...
      try {
        ...
        pane.setText(URLDecoder.decode(location.substring(HtmlDisplay.stringPrefix.length()), "utf-8"));
      } catch (java.io.UnsupportedEncodingException e) {
        throw new IllegalStateException("Encodage non reconnu : (" + e + ") c'est un bug Java !");
      }
      ...
      // editeur
      // JavaScool
      Desktop.getInstance().openFile(toURL(location.substring(HtmlDisplay.editorPrefix.length())));
    } else if (location.startsWith(HtmlDisplay.newtabPrefix)) { // Affichage
      // dans
      // browser
      // JavaScool
      URL url = toURL(location.substring(HtmlDisplay.newtabPrefix.length()));
      String name = new File(url.getPath()).getName().replaceFirst("\\.[^\\.]*$", "").replace('_', '.');
      org.javascool.main.Desktop.getInstance().openBrowserTab(url.toString(), name.substring(0, 1).toUpperCase() + name.substring(1));
  ...
  private URL toURL(String location) {
    try {
      return urls.isEmpty() ? Macros.getResourceURL(location) : urls.getCurrent() instanceof URL ? new URL((URL) urls.getCurrent(), location) : new URL(location);
    } catch (MalformedURLException e) {
      try {
        return new URL(HtmlDisplay.stringPrefix + "Le lien : <tt>«" + location + "»</tt> est mal formé");
