...
*/
  public final IconOutput reset(int width, int height, boolean zoom) {
    ...
    this.zoom = zoom;
    /*
     * if (width <= 0) width = 300; if (height <= 0) height = 300; if(width % 2
     * == 0) width++; if(height % 2 == 0) height++;
     */
...
*/
  public IconOutput reset(String location, boolean zoom) throws IOException {
    ...
    for (int n = 0; n < 3; n++) {
      ...
      if (img != null) return reset(img, zoom);
    }
...
*/
  public String getColor(int x, int y) {
    if ((0 <= x) && (x < width) && (0 <= y) && (y < height)) {
      Color c = image[x + y * width];
      return IconOutput.colors.containsKey(c) ? IconOutput.colors.get(c) : c.toString();
    } else
      return "undefined";
  }
