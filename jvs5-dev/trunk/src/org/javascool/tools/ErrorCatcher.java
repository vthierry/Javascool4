...
public class ErrorCatcher {
  ...
  /** Ouvre une fenêtre d'alerte en cas d'exception intempestive et non prise en
   * compte.
   * <p>
   * Installe un gestionnaire d'exception non interceptée qui recueille des
   * informations sur: les versions des composants logiciels, le nom du process,
   * la trace de la pile et l'affiche dans une fenêtre séparée afin d'être
   * recueillies et communiquées par l'utilisateur.
   * </p>
   * 
   * @param header Un texte entête en HTML expliquant à l'utilisateur quoi faire
   *        avec cette sortie d'exception.
   * @param revision Nom et/ou numéro de révision de l'application pour avoir
  ...
  public static void setUncaughtExceptionAlert(String header, String revision, String keyword) {
    ...
    Thread.setDefaultUncaughtExceptionHandler(new Thread.UncaughtExceptionHandler() {
      ...
      public void uncaughtException(Thread t, Throwable e) {
        ...
        boolean alert = (ErrorCatcher.uncaughtExceptionKeyword == null || r.indexOf(ErrorCatcher.uncaughtExceptionKeyword) != -1) && (e.toString().indexOf("java.util.ConcurrentModificationException") == -1) && (e.toString().indexOf("com.sun.java.swing.plaf.nimbus.DerivedColor$UIResource cannot be cast to com.sun.java.swing.Painter") == -1);
        s += r + "</pre><hr>";
