...
*/
  public String getApplicationFolder() {
    ...
    else if (OS.contains("MAC")) return System.getProperty("user.home") + "/Library/Application Support/" + applicationName + "/";
    else if (OS.contains("NUX")) return System.getProperty("user.home") + "/." + applicationName + "/";
    else {
  ...
  private void loadConfiguration(){
    if(properties!=null) return; // Le fichier est déjà chargé
    if(!new File(getConfigurationFileLocation()).exists()){ // Le fichier n'existe pas
...
*/
  public static UserConfig getInstance(String applicationName) {
    if (UserConfig.userConfig == null) return UserConfig.userConfig = new UserConfig(UserConfig.theApplicationName = applicationName);
      ...
      throw new IllegalArgumentException("Appel incohérent à la configuration de l'application avec deux noms différents " + UserConfig.theApplicationName + " puis " + applicationName);
  }
  ...
  public static String getOS() {
    ...
    if (os.indexOf("mac") >= 0) return MAC_OSX;
    return LINUX;
  }
