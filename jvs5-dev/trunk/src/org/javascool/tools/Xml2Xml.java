...
public class Xml2Xml {
   ...
   * @param xsl Le nom de fichier ou la chaîne avec les règles de transformation
   *        XSL.
   *        <p>
   *        - Si la chaîne commence par un <tt>&lt;</tt> elle est reconnue comme
   *        un texte XSLT.
   *        </p>
   *        <p>
   *        - Sinon elle est reconnue comme un non de fichier.
   *        </p>
   * @param params Paramètres de la transformation. La valeur null indique qu'il
...
*/
   ...
   * <p>
   * Elimine les entitées HTML connues (tout n'est pas implémenté) et passe à de
   * l'accentuation liée au codage local, élimine les constructions
   * (commentaires, instructions) qui ne sont pas structures logiques XML et
   * ferme les balises pur avoir une syntaxe bien formée.
   * </p>
   * <p>
   * C'est une commande "fragile" au sens où un text HTML mal formé ne sera pas
...
*/
  public static void main(String[] usage) {
    ...
    if (usage.length == 5) {
      ...
      FileManager.save(usage.length > 2 ? usage[2] : "stdout:", Xml2Xml.run(FileManager.load(usage[0]), usage[1]));
    }
  }
}
