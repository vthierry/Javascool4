...
public class Pml {
  ...
  /** Initialise la PML en la lisant dans une chaîne de caractères dans un format
   * donné.
   * 
   * @param value La chaîne de syntaxe
   *        <tt>"{tag name = value .. element .. }"</tt>.
   * @param format Les formats possible sont: <div id="input-format">
   *        <ul>
  ...
  public Pml reset(String value, String format) {
    ...
    else if ("htm".equals(format) || "html".equals(format)) return reset(Xml2Xml.run(Xml2Xml.html2xhtml(value), Pml.xml2pml), "pml");
    else {
      reset(value);
    }
    return this;
   ...
   * 
   * @param format de sortie <div id="output-format">
   *        <ul>
   *        <li>"RAW" Retourne une chaîne 1D de longueur minimale (par défault).
   *        </li>
   *        <li>"TXT" Retourne une chaîne 2D formattée.</li>
   *        <li>"XML" Retourne une structure logique XML, en réduisant les tag
   *        et attributs à des nom XML valides et en considérant les PML sans
   *        attribut ni élément comme des chaînes.
   *        <li>
   *        <li>"PHP" Retourne un élément de code PHP de la forme:
   *        <tt>&lt;php $tag = array("_tag" = getTag(), . . "name" => "value", . . , "element");?></tt>
   *        .</li>
   *        <li>"JMF" Retourne un format de fichier de manifeste de JAR de la
   *        forme: <tt> name : value \n .. </tt> en omettant le tag et les
   *        éléments.</li>
   *        </ul>
   *        </div>
   ...
   * 
   * @param location L'URL (Universal Resource Location) de chargement de <a
   *        href="FileManager.html#load-format">format standard</a>.
   * @param format Le format de lecture parmi les <a
   *        href="#input-format">formats supportés</a>, par défaut donné par
   ...
   * 
   * @param location L'URL (Universal Resource Location) d'écriture de <a
   *        href="FileManager.html#save-format">format standard</a>.
   * @param format Le format d'écriture parmi les <a
  ...
  private static class XmlWriter {
    ...
    private void write(Pml pml) {
      if (pml.getSize() == 0) {
        ...
        for (String name : pml.attributes()) {
          string.append(" ").append(Pml.toName(name)).append("=\"").append(pml.getChild(name).toString().replaceFirst("^\\{(.*)\\}$", "$1").replaceFirst("^\"(.*)\"$", "$1").replaceAll("&", "&amp;").replaceAll("<", "&lt;").replaceAll("\"", "&quot;")).append("\"");
        }
        if (pml.getCount() > 0) {
          string.append(">");
          for (int n = 0; n < pml.getCount(); n++) {
          ...
          string.append("</").append(Pml.toName(pml.getTag())).append(">");
        } else {
          string.append("/>");
        }
      }
  ...
  private static class PhpWriter {
    ...
    public String toString(Pml pml) {
      ...
      if (pml == null) return "<?php $pml = array(); ?>";
        ...
        string.append("<?php $").append(tag).append(" = array(\"_tag\" => ").append(PhpWriter.quote(pml.getTag()));
        for (String name : pml.attributes()) {
          string.append(", ").append(PhpWriter.quote(name)).append(" => ").append(PhpWriter.quote(pml.getChild(name)));
        }
        for (int n = 0; n < pml.getCount(); n++) {
          string.append(", ").append(PhpWriter.quote(pml.getChild(n)));
        }
        string.append("); ?>");
      }
      return string.toString();
  ...
  private static class JmfWriter {
    ...
    public String toString(Pml pml) {
      ...
      if (pml == null) return "";
        ...
        for (String name : pml.attributes()) {
          string.append(name).append(": ").append(JmfWriter.quote(pml.getChild(name))).append("\n");
        }
      }
      return string.toString();
    }
  ...
  /** Renvoie la d'un paramètre de ce PML en tant qu'objet Java. /** Renvoie la
   * valeur d'un paramètre de ce PML en tant que chaîne.
   * 
   * @param name Le nom de l'attribut ou l'index de l'élément (sous forme de
   *        chaîne ou d'entier).
  ...
  public final boolean getBoolean(String name, boolean value) {
    ...
    if (v != null) {
      if ("true".equals(v.toLowerCase())) return true;
      if ("false".equals(v.toLowerCase())) return false;
    }
    return value;
  }
      ...
      public Iterator<String> iterator() {
        return new Iterator<String>() {
          ...
          private void nextKey() {
            for (key = null; keys.hasNext() && Pml.isIndex(key = keys.next()); key = null) {
            }
          }
