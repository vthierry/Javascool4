...
*/
  public void setProglet(String name) {
    ...
    if (proglet.getPane() != null) {
      this.addTab("Proglet " + name, "", new JScrollPane(proglet.getPane()));
      this.progletTabId = "Proglet " + name;
    }
    if (proglet.getHelp() != null) {
      this.addTab("Aide de la proglet", "", new HtmlDisplay().setPage(Macros.getResourceURL(proglet.getHelp())));
...
*/
  ...
  /** Ouvre un nouvel onglet web Ouvre un nouveau HTMLDisplay dans un onglet. Cet
   * onglet peut être fermer à l'aide de la croix qui se situe à droite du titre
   * de l'onglet.
   * 
