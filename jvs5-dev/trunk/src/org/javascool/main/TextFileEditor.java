...
class TextFileEditor extends TextEditor {
  ...
  public boolean load() {
    try {
      ...
      try {
        ...
        JOptionPane.showMessageDialog(null, "Le fichier ne peut pas être lu, il est inaccessible en tant que fichier texte.", "Erreur de lecture", JOptionPane.ERROR_MESSAGE);
        return false;
      }
    } catch (IllegalStateException e) { // L'utilisateur a annulé la demande
      return false;
    } /*
       * JFileChooser fc = new JFileChooser(); String dir =
       * TextFileEditor.getWorkingDir(); if (dir != null) {
       * fc.setCurrentDirectory(new File(dir)); } int returnVal =
       ...
       * .getAbsolutePath()); try {
       * load(fc.getSelectedFile().getAbsolutePath()); return true; } catch
       ...
       * , "Erreur de lecture", JOptionPane.ERROR_MESSAGE); return false; } }
       * else return false;
       */
  }
  ...
  private static String getWorkingDir() {
    if (UserConfig.getInstance("javascool").getProperty("dir") != null) return UserConfig.getInstance("javascool").getProperty("dir");
    else if (System.getProperty("os.name").toLowerCase().contains("nix") || System.getProperty("os.name").toLowerCase().contains("nux")) return System.getProperty("user.dir");
    else if (System.getProperty("home.dir") != null) return System.getProperty("home.dir");
    else
      return null;
  }
  ...
  public boolean saveAs() {
     ...
     * fc.setApproveButtonText("Enregistrer"); fc.setDialogTitle("Enregistrer");
     * int returnVal = fc.showOpenDialog(this.getParent()); if (returnVal ==
     * JFileChooser.APPROVE_OPTION) {
     * TextFileEditor.setWorkingDir(fc.getSelectedFile().getParentFile()
     * .getAbsolutePath()); save(fc.getSelectedFile().getAbsolutePath()); return
  ...
  public boolean save(boolean confirm) {
    if (isTextModified()) {
      if (confirm) {
        int r = JOptionPane.showConfirmDialog(null, "Voulez vous enregistrer " + getName() + " avant de continuer ?");
        if (r == JOptionPane.YES_OPTION) return save(false);
        else
          return r == JOptionPane.NO_OPTION;
      } else {
        try {
          if (location == null) return saveAs();
          else {
            long l = FileManager.getLastModified(location);
            if (lastModified != 0 && l != 0 && lastModified < l) {
              int r = JOptionPane.showConfirmDialog(null, "Le fichier " + getName() + " a été modifié par un autre logiciel : voulez-vous écraser ces modifications ?");
              if (r == JOptionPane.NO_OPTION) return saveAs();
              else if (r != JOptionPane.YES_OPTION) return false;
            }
            save(location);
            return true;
          }
        } catch (Exception e) {
          JOptionPane.showMessageDialog(null, "Le fichier ne peut pas être écrit ici, choisisez un nouvel endroit.", "Erreur d'écriture", JOptionPane.ERROR_MESSAGE);
          return false;
        }
      }
    } else
   ...
   *        <tr>
   *        <td><tt>file:/<i>path-name</i></tt></td>
   *        <td>pour sauver dans le système de fichier local (le <tt>file:</tt>
   *        est optionnel).</td>
   *        </tr>
   *        <tr>
