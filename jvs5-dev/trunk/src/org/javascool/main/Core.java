...
public class Core {
   ...
   * @throws RuntimeException lorsque l'application n'a pas été démarré depuis
   *         un jar */
  public static String javascoolJar() {
    if (Core.javascoolJar != null) return Core.javascoolJar;
    ...
    if (url.endsWith(".jar")) {
      try {
        ...
        if (new File(jar).exists()) return Core.javascoolJar = jar;
