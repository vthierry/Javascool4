...
*/
  ...
  private boolean canOpen(TextFileEditor file) {
    ...
    if (!ok) {
      int r = JOptionPane.showConfirmDialog(null, "Le fichier " + file.getName() + " est déjà ouvert, voulez-vous vraiment éditer «sur» l'autre version ?", "Erreur d'ouverture", JOptionPane.YES_NO_OPTION);
  ...
  private TextFileEditor getSelectedFile(boolean verbose) {
    if (getSelectedIndex() != -1 && getComponentAt(getSelectedIndex()) instanceof TextFileEditor) return selectedFile = (TextFileEditor) getComponentAt(getSelectedIndex());
    else {
      if (verbose) {
        JOptionPane.showMessageDialog(null, "Il n'y a aucun fichier sélectionné !", "Erreur", JOptionPane.ERROR_MESSAGE);
      }
      return null;
    }
  }
...
*/
  ...
  void openFile(URL url) {
    if (getSelectedFile(false) == null) return;
