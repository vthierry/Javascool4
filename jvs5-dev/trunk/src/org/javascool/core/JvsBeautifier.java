...
public class JvsBeautifier {
  ...
  public static String run(String text) {
    ...
    for (int i = 0; i < f.length;) {
      ...
      if ((f[i] == '/') && (i < f.length - 1) && (f[i + 1] == '*')) {
        ...
        if (JvsBeautifier.isOperator(f[i])) {
          if (!(Character.isWhitespace(c0) || JvsBeautifier.isOperator(c0))) {
            g += ' ';
          }
          g += f[i];
          if ((i < f.length - 1) && !(Character.isWhitespace(f[i + 1]) || JvsBeautifier.isOperator(f[i + 1]))) {
            g += ' ';
          }
        } else if (f[i] == '.') {
          ...
          g += f[i];
          if ((par > 0) && (f[i] != ')')) if ((i < f.length - 1) && !Character.isWhitespace(f[i + 1])) {
            g += ' ';
          }
          if ((f[i] == ')') && (i < f.length - 1) && (f[i + 1] == '{')) {
            g += ' ';
    ...
    return "\n" + g.replaceAll("\\}\\s*else\\s*(\\{|if)", "} else $1").replaceAll("(while|if|for|return)\\s*([^a-z_0-9_])", "$1 $2");
  }
