...
import org.javascool.main.Core;
...
*/
  public static boolean compile(String jarFile, String mainClass, String javaFiles[], boolean allErrors) {
    try {
      ...
      if (!Java2Class.compile(javaFiles, allErrors)) return false;
      String mfFile = buildDir + File.separator + "manifest.jmf";
      new Pml().set("Main-Class", mainClass).set("Implementation-URL", "http://javascool.gforge.inria.fr").save(mfFile);
...
*/
  public static void main(String[] usage) {
    ...
    if (usage.length > 1) {
      ...
      Java2Jar.compile(usage[usage.length - 1], new File(usage[0]).getName().replaceAll("\\.java$", ""), javaFiles, true);
    }
  }
}
