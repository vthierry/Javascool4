...
public class Java2Class {
   ...
   * Initialisation des objets dy compilateur// The compiler tool JavaCompiler
   * compiler = ToolProvider.getSystemJavaCompiler(); // The compiler tool if
   * (compiler == null) { System.err.println(
   ...
   * Locale.FRENCH, null); // The file manager // Mise en place des fichiers
   * List<File> sourceFileList = new ArrayList<File>(); for(String javaFile :
   * javaFiles) sourceFileList.add(new File(javaFile)); Iterable< ? extends
   * JavaFileObject> compilationUnits =
   * fileManager.getJavaFileObjectsFromFiles(sourceFileList); // Lancement de la
   * compilation JavaCompiler.CompilationTask task = compiler.getTask(null,
   ...
   * +" erreur="+javaDiagnostic); // En fait ici on choisit d'arrêter à la 1ère
   * erreur pour pas embrouiller l'apprennant
   * if(diagnostic.getKind().equals(Diagnostic.Kind.ERROR) && !allErrors) return
   * false; } return true; }
   */
  private static boolean compile2(String javaFiles[], boolean allErrors) {
    ...
    Method javac;
    try {
      javac = Class.forName("com.sun.tools.javac.Main").getDeclaredMethod("compile", Class.forName("[Ljava.lang.String;"), Class.forName("java.io.PrintWriter"));
    } catch (Exception e) {
    ...
    // Traitement du message de sortie
    {
      ...
      sout = sout.replaceAll("not a statement", "L'instruction n'est pas valide.\n (Il se peut qu'une variable indiquée n'existe pas)");
      sout = sout.replaceAll("';' expected", "Un ';' est attendu (il peut manquer, ou une parenthèse être incorrecte, ..)");
      sout = sout.replaceAll("cannot find symbol\\s*symbol\\s*:\\s*([^\\n]*)[^:]*:\\s*(.*)", "Il y a un symbole non-défini à cette ligne : «$1» (utilisez-vous la bonne proglet ?)");
      sout = sout.replaceAll("illegal start of expression", "($0) L'instruction (ou la précédente) est tronquée ou mal écrite");
      sout = sout.replaceAll("class, interface, or enum expected", "($0) Il y a probablement une erreur dans les accolades (peut-être trop de '}')");
      sout = sout.replaceAll("'.class' expected", "($0) Il manque des accolades ou des parenthèses pour définir l'instruction");
      sout = sout.replaceAll("incompatible\\Wtypes\\W*found\\W*:\\W([A-Za-z\\.]*)\\Wrequired:\\W([A-Za-z\\.]*)", "Vous avez mis une valeur de type $1 alors qu'il faut une valeur de type $2");
   ...
   * 
   * @param path Le chemin vers la classe Java à charger. La classe ne doit pas
