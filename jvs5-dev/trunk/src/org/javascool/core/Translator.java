...
public class Translator {
  ...
  /** Transforme globalement le code pour passer des constructions spécifiques à
   * Jvs à du java standard.
   * <p>
   * Ce sont souvent des expression régulières appliquées à la chaîne, tout est
   * ici de la responsabilité du concepteur de la proglet.
   * </p>
   * <p>
   ...
   * <p>
   * <tt>scanConstruct(string, offset, "(", ")", null, true, true)</tt> cerne un
   * parenthésage dans un code.
   * </p>
   * <p>
   * <tt>scanConstruct(string, offset, "'", "'", "\'", false, false)</tt> cerne
   * une chaîne entre quotes ' '.
   * </p>
   * 
   ...
   * @param escape Le symbole d'échappement de la parenthèse fermante, ou null
   *        si non défini.
   ...
   *        <tt>/* . . *</tt><tt>/</tt> ou <tt>// . . /n</tt> et les chaînes de
   *        caractères <tt>" . . "</tt>, si égal à true.
   * @return Renvoie int[2], l'index du premier caratère et un plus l'index du
   *         dernier caractères du parenthésage,
   *         <p>
   *         tel que <tt>string.substring(index[0], index[1])</tt> corresponde à
   *         la chaîne identifiée, sinon
   *         </p>
   *         <p>
  ...
  public static int[] scanConstruct(String string, int offset, String start, String stop, String escape, boolean recurse, boolean code) {
    ...
    if (0 <= offset && offset < string.length() && stop != null) {
      ...
      if (!(index[0] < string.length() && (start == null || string.startsWith(start, index[0])))) return new int[] { -1, offset };
      for (index[1] = index[0] + (start == null ? 0 : start.length()); index[1] < string.length() && !string.startsWith(stop, index[1]);) {
        if (escape != null && escape.length() > 0 && string.startsWith(escape, index[1])) {
          ...
          int next[] = Translator.scanConstruct(string, index[1], start, stop, escape, recurse, code);
          if (next[0] == -1) return new int[] { -1, offset };
          else {
            index[1] = next[1];
          }
        } else if (code) {
  ...
  public static int[] scanInstruction(String string, int offset) {
    ...
    if (index[0] == -1) {
      index = Translator.scanConstruct(string, offset, null, ";", null, false, true);
    }
    return index;
  }
   ...
   * portion de texte.
   * <p>
   * Passe par dessus les commentaires de programme <tt>/* . . *</tt> <tt>/</tt>
   * ou <tt>// . . /n</tt> et les chaînes de caractères <tt>" . . "</tt>.
   * </p>
   * 
   * @param string Le contenu du texte à analyser.
   * @param offset L'index de départ dans le texte à analyser.
  ...
  public static int scanComments(String string, int offset, boolean next) {
    ...
    for (boolean loop = true; loop;) {
      ...
      offset = Translator.scanConstruct(string, offset, "/*", "*/", null, false, false)[1];
