...
public class Macros {
  ...
  /** Excécute une routine à un intervalle régulier. Exemple d'usage (impression
   * de 10 messages à interval d'1 sec. puis arrêt):
   * 
   * <pre>
   ...
   * <p>
   * La fonction recherche l'existence du fichier: (i) par rapport au répertoire
   * de base qui est donné, (ii) par rapport au dossier de travaul "user.dir",
   * (iii) par rapport à la racine des fichier "user.home", (iv) dans les
   * ressources du CLASSPATH.
   * </p>
   * 
  ...
  public static URL getResourceURL(String location, String base, boolean reading) {
    ...
    try {
      ...
      if (location.matches("(ftp|http|https|jar|mailto|stdout):.*")) return new URL(location).toURI().normalize().toURL();
