...
public class Calculator extends Frame implements ItemListener, WindowListener{
	...
	//	resolution.hide();
	    ...
	    c.gridwidth=1;c.gridheight=1;c.weightx=c.weighty=0;
    ...
    private void calculate(){
		...
		if (selected != null){
			...
			if (selectedVal == 24){depth = 24;}
			else if (selectedVal == 256){depth = 8;}
			else if (selectedVal == 16){depth = 4;}
			else if (selectedVal == 4){depth = 2;}
			else if (selectedVal == 2){depth = 1;}
			else{depth=24;}
		    }
		}else{
		    depth=24;
