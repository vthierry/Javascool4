 ...
 * @serial exclude
 */
public class SoundDisplay implements org.javascool.Proglet {
  ...
  private static class Panel extends SoundBitPanel {
    ...
    public SoundBit sound = new NotesSoundBit() {
      public double get(char channel, double time) {
        return SoundDisplay.tone == null ? Math.sin(2 * Math.PI * time) : SoundDisplay.tone.get(channel, time);
      }
    };
    public Panel() {
      reset(sound, 'l');
  ...
  static public void syntheSet(String notes) {
    panel.sound.reset(notes);
    panel.reset(panel.sound, 'l');
  }
  /** Fait entendre le son sur le système audio. */
  static public void synthePlay() {
    try { panel.sound.play();
    } catch(Exception e) {
      Macros.echo(e.toString());
    }
  }
