...
public class Classe extends org.javascool.proglets.plurialgo.langages.modele.Classe {
	...
	public Classe(String nom, boolean isRecord) {
		this.nom = nom;
		...
		for(Iterator<InfoTypee> iter=infos.iterator(); iter.hasNext(); ) {
			...
			if (info_prop==null) {	// ce n'est pas une propriete
				...
				if (liste.getInfo(info.nom)==null) {
					prop = new Variable(info.nom, info.type, null);
					proprietes.add(prop);
	...
	void fusionner(Classe cl) {
		if (!this.nom.equals(cl.nom)) return;
		for(Iterator<org.javascool.proglets.plurialgo.langages.modele.Operation> iter_oper=cl.operations.iterator();iter_oper.hasNext();) {
			Operation oper = (Operation) iter_oper.next();
			if (this.getOperation(oper.nom)==null) {
				this.operations.add(oper);
			}
		}
		for(Iterator<org.javascool.proglets.plurialgo.langages.modele.Variable> iter_prop=cl.proprietes.iterator();iter_prop.hasNext();) {
