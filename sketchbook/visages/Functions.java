...
public class Functions {
  ...
  public static void calcul_vecteurs_propres(float[][] covariance, int nbvecteurspropres) {
    ...
    while(num < nombre) {
       ...
       *  double s= 0;
       *  for(int col = 0; col<dimension;col++){
       *  s = s+ covariance[lin][col]*image_propre[col][0];
       *  }
       *  result[lin] = s;
       *  System.out.println(image0[lin]+"||"+ result[lin]+"|"+  val_propre[num]*image0[lin] );
       *  }*/
    ...
    sauvegarde_text(image_propre, valeurs_propres);
  }
    ...
    for(int i = 0; i < height1; i++)
      for(int j = 0; j < width1; j++) {
        ...
        if((rouge == bleu) && (bleu == vert)) {
          luma = rouge;
        } else { luma = (short) ((77 * rouge + 151 * vert + 28 * bleu) / 256);
        }                                                    // ((bleu + rouge +vert)/3);}*/
