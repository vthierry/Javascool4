...
public class DialogFrame {
	...
	private static void startConvertisseurHML() {
		final JPopupMenu jCreatorMenu = new JPopupMenu();
		jCreatorMenu.add(new Htm2Hml());
		final Component parent = Console.getInstance().getToolBar();
	...
	private static void startProgletBuilderMenu() {
		...
		if (DialogFrame.jBuilderMenu == null || reload) {
			...
			if (ProgletsBuilder.getProglets().length > 0) {
				...
				for (final String proglet : ProgletsBuilder.getProglets()) {
					final JCheckBox check = new JCheckBox(proglet);
					check.setSelected(true);
					DialogFrame.jBuilderMenu.add(check);
				}
				DialogFrame.jBuilderMenu.addSeparator();
				final JMenuItem menuitem = new JMenuItem("Construire le jar");
					...
					public void actionPerformed(ActionEvent e) {
						new Thread(new Runnable() {
							...
							public void run() {
								...
								for (final Component c : DialogFrame.jBuilderMenu
										.getComponents()) {
		...
		menuitem.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				new Thread(new Runnable() {
					...
					@Override
					public void run() {
						final String name = DialogFrame.jCreatorMenuName
								.getText();
