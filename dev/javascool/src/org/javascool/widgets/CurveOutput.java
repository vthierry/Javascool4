...
public class CurveOutput extends JPanel {
	...
	{
		final ReticuleMouseListener l = new ReticuleMouseListener();
	...
	public void paint(Graphics g) {
		...
		g.setPaintMode();
		for (int c = 0; c < 10; c++) {
			final ArrayList<point> curve = curves.get(c);
			g.setColor(CurveOutput.colors[c]);
			for (int i = 1; i < curve.size(); i++) {
				final point p0 = curve.get(i - 1), p1 = curve.get(i);
		...
		for (final oval l : ovals) {
			g.setColor(l.c);
			g.drawOval(x2i(l.x), y2j(l.y), x2w(l.w), y2h(l.h));
		}
		for (final label l : labels) {
			final int i = x2i(l.x), j = y2j(l.y);
			g.setColor(l.c);
	...
	private void paintReticule(Graphics g) {
		final int i = x2i(reticuleX), j = y2j(reticuleY);
		g.setColor(Color.white);
